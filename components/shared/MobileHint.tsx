...
export function MobileHint({
  ...
  // Panorama swipe overlay: show only once per session
  useEffect(() => {
  ...
  // Regular pill hint: show on every viewpoint change on mobile
  useEffect(() => {
    setShowPill(false);
  ...
  return (
    <>
      {/* Panorama swipe hint overlay — once per session */}
      ...
      {/* Regular pill hint — every viewpoint stop */}
      {showPill && (
